...
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await update.message.reply_text('👋 Бот для відмітки часу', reply_markup=InlineKeyboardMarkup(keyboard))
...
async def settings(update: Update):
    ...
    msg = (f'🎨 Бібліотека:\n\n'
           f'✅ Check-in: {len(media["checkin"])}\n'
           f'🚪 Check-out: {len(media["checkout"])}')
...
async def show_workload(update: Update):
    keyboard = [
        [InlineKeyboardButton("🟢 Потрібні задачі", callback_data='w_🟢')],
        [InlineKeyboardButton("🟡 Середня завантаженість", callback_data='w_🟡')],
        [InlineKeyboardButton("🔴 Завантаженість до пенсії", callback_data='w_🔴')],
        [InlineKeyboardButton("➡️ Пропустити", callback_data='w_skip')]
    ]
    await update.callback_query.answer()
    try: await update.callback_query.message.delete()
    except: pass
